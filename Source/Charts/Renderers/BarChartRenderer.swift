...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    open func drawGradient(context: CGContext, barRect: CGRect, gradientColors: Array<NSUIColor>, orientation: BarGradientOrientation)
    {
        ...
        if gradientColors.count == 3{
            gradient = CGGradient(colorsSpace: CGColorSpaceCreateDeviceRGB(), colors: cgColors, locations: [0.0,0.2,1.0])
        }else{
            gradient = CGGradient(colorsSpace: CGColorSpaceCreateDeviceRGB(), colors: cgColors, locations: nil)
        }
